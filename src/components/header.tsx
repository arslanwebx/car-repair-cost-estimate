...
export function Header() {
  ...
  return <header className="site-header"><div className="shell header-inner"><Logo/><nav className={open ? "nav-open" : ""} aria-label="Main navigation" id="mobile-navigation">{links.map(link => { const path = link.href.split("#")[0]; const current = !link.href.includes("#") && (path === "/" ? pathname === "/" : pathname.startsWith(path)); return <Link key={link.href} href={link.href} aria-current={current ? "page" : undefined} onClick={()=>setOpen(false)}>{link.label}</Link>; })}</nav><button ref={button} type="button" className="menu-button" aria-expanded={open} aria-controls="mobile-navigation" aria-label={open ? "Close navigation" : "Open navigation"} onClick={()=>setOpen(value=>!value)}><span/><span/><span/></button><Link className="button button-small header-cta" href="/estimate">Get Free Estimate</Link></div></header>;
}
